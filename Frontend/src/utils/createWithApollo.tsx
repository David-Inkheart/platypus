...
export default function createWithApollo<P, IP>(ac: ApolloClientParam) {
  return ({ ssr = false } = {}) => (PageComponent: NextPage<P, IP>) => {
    const createWithApollo = (pageProps: P & WithApolloOptions) => {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      createWithApollo.displayName = `createWithApollo(${displayName})`;
    }

    if (ssr || PageComponent.getInitialProps) {
      createWithApollo.getInitialProps = async (ctx: ContextWithApolloOptions) => {
        const inAppContext = Boolean(ctx.ctx);
